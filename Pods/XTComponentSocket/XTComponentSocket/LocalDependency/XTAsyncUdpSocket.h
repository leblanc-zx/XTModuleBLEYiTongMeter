...
extern NSString *const XTAsyncUdpSocketException;
extern NSString *const XTAsyncUdpSocketErrorDomain;

extern NSString *const XTAsyncUdpSocketQueueName;
extern NSString *const XTAsyncUdpSocketThreadName;

typedef NS_ENUM(NSInteger, XTAsyncUdpSocketError) {
	XTAsyncUdpSocketNoError = 0,          // Never used
	XTAsyncUdpSocketBadConfigError,       // Invalid configuration
	XTAsyncUdpSocketBadParamError,        // Invalid parameter was passed
	XTAsyncUdpSocketSendTimeoutError,     // A send operation timed out
	XTAsyncUdpSocketClosedError,          // The socket was closed
	XTAsyncUdpSocketOtherError,           // Description provided in userInfo
};
...
@class XTAsyncUdpSocket;

@protocol XTAsyncUdpSocketDelegate <NSObject>
@optional
...
**/
- (void)udpSocket:(XTAsyncUdpSocket *)sock didConnectToAddress:(NSData *)address;
...
**/
- (void)udpSocket:(XTAsyncUdpSocket *)sock didNotConnect:(NSError * _Nullable)error;

/**
 * Called when the datagram with the given tag has been sent.
**/
- (void)udpSocket:(XTAsyncUdpSocket *)sock didSendDataWithTag:(long)tag;

/**
 * Called if an error occurs while trying to send a datagram.
 * This could be due to a timeout, or something more serious such as the data being too large to fit in a sigle packet.
**/
- (void)udpSocket:(XTAsyncUdpSocket *)sock didNotSendDataWithTag:(long)tag dueToError:(NSError * _Nullable)error;

/**
 * Called when the socket has received the requested datagram.
**/
- (void)udpSocket:(XTAsyncUdpSocket *)sock didReceiveData:(NSData *)data
...
**/
- (void)udpSocketDidClose:(XTAsyncUdpSocket *)sock withError:(NSError  * _Nullable)error;
 ...
 * Example:
 * 
 * XTAsyncUdpSocketReceiveFilterBlock filter = ^BOOL (NSData *data, NSData *address, id *context) {
 * 
 ...
 * 
**/
typedef BOOL (^XTAsyncUdpSocketReceiveFilterBlock)(NSData *data, NSData *address, id __nullable * __nonnull context);
 ...
 *
**/
typedef BOOL (^XTAsyncUdpSocketSendFilterBlock)(NSData *data, NSData *address, long tag);


@interface XTAsyncUdpSocket : NSObject

/**
 * XTAsyncUdpSocket uses the standard delegate paradigm,
 ...
 * If you pass NULL, XTAsyncSocket will automatically create its own socket queue.
...
- (instancetype)initWithDelegate:(nullable id <XTAsyncUdpSocketDelegate>)aDelegate delegateQueue:(nullable dispatch_queue_t)dq;
- (instancetype)initWithDelegate:(nullable id <XTAsyncUdpSocketDelegate>)aDelegate delegateQueue:(nullable dispatch_queue_t)dq socketQueue:(nullable dispatch_queue_t)sq;

#pragma mark Configuration

- (nullable id <XTAsyncUdpSocketDelegate>)delegate;
- (void)setDelegate:(nullable id <XTAsyncUdpSocketDelegate>)delegate;
- (void)synchronouslySetDelegate:(nullable id <XTAsyncUdpSocketDelegate>)delegate;
...
- (void)getDelegate:(id <XTAsyncUdpSocketDelegate> __nullable * __nullable)delegatePtr delegateQueue:(dispatch_queue_t __nullable * __nullable)delegateQueuePtr;
- (void)setDelegate:(nullable id <XTAsyncUdpSocketDelegate>)delegate delegateQueue:(nullable dispatch_queue_t)delegateQueue;
- (void)synchronouslySetDelegate:(nullable id <XTAsyncUdpSocketDelegate>)delegate delegateQueue:(nullable dispatch_queue_t)delegateQueue;

/**
 * By default, both IPv4 and IPv6 are enabled.
 * 
 * This means XTAsyncUdpSocket automatically supports both protocols,
 * and can send to IPv4 or IPv6 addresses,
 * as well as receive over IPv4 and IPv6.
 * 
 * For operations that require DNS resolution, XTAsyncUdpSocket supports both IPv4 and IPv6.
 * If a DNS lookup returns only IPv4 results, XTAsyncUdpSocket will automatically use IPv4.
 * If a DNS lookup returns only IPv6 results, XTAsyncUdpSocket will automatically use IPv6.
 ...
 * 
 * Since the OS/XT notifies us of the size of each received UDP packet,
 ...
 * This is due to the fact that XTAsyncUdpSocket does NOT copy the data.
 ...
 *    or the system tools to handle this aren't available (e.g. on a mobile device).
 * 
 * For more information about XTAsyncUdpSocketSendFilterBlock, see the documentation for its typedef.
...
**/
- (void)setSendFilter:(nullable XTAsyncUdpSocketSendFilterBlock)filterBlock withQueue:(nullable dispatch_queue_t)filterQueue;
...
**/
- (void)setSendFilter:(nullable XTAsyncUdpSocketSendFilterBlock)filterBlock
 ...
 * 
 * Important Note:
 * XTAsyncUdpSocket may be running in parallel with your code.
 * That is, your delegate is likely running on a separate thread/dispatch_queue.
 * When you invoke this method, XTAsyncUdpSocket may have already dispatched delegate methods to be invoked.
 ...
 * [udpSocket setReceiveFilter:filter withQueue:myParsingQueue];
 * 
 * For more information about XTAsyncUdpSocketReceiveFilterBlock, see the documentation for its typedef.
 * To remove a previously set filter, invoke this method and pass a nil filterBlock and NULL filterQueue.
 * 
 * Note: This method invokes setReceiveFilter:withQueue:isAsynchronous: (documented below),
 *       passing YES for the isAsynchronous parameter.
**/
- (void)setReceiveFilter:(nullable XTAsyncUdpSocketReceiveFilterBlock)filterBlock withQueue:(nullable dispatch_queue_t)filterQueue;
 ...
 * For example, you can't query properties on the socket.
**/
- (void)setReceiveFilter:(nullable XTAsyncUdpSocketReceiveFilterBlock)filterBlock
               withQueue:(nullable dispatch_queue_t)filterQueue
          isAsynchronous:(BOOL)isAsynchronous;
 ...
 * 
 * The XTAsyncUdpSocket instance may optionally be used again.
 ...
 * 
 * Generally XTAsyncUdpSocket doesn't use CFStream. (It uses the faster XT API's.)
