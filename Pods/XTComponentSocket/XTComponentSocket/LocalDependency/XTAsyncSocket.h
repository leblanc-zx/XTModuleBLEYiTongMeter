...
@class XTAsyncReadPacket;
@class XTAsyncWritePacket;
@class XTAsyncSocketPreBuffer;
@protocol XTAsyncSocketDelegate;

NS_ASSUME_NONNULL_BEGIN

extern NSString *const XTAsyncSocketException;
extern NSString *const XTAsyncSocketErrorDomain;

extern NSString *const XTAsyncSocketQueueName;
extern NSString *const XTAsyncSocketThreadName;

extern NSString *const XTAsyncSocketManuallyEvaluateTrust;
#if TARGET_OS_IPHONE
extern NSString *const XTAsyncSocketUseCFStreamForTLS;
#endif
#define XTAsyncSocketSSLPeerName     (NSString *)kCFStreamSSLPeerName
#define XTAsyncSocketSSLCertificates (NSString *)kCFStreamSSLCertificates
#define XTAsyncSocketSSLIsServer     (NSString *)kCFStreamSSLIsServer
extern NSString *const XTAsyncSocketSSLPeerID;
extern NSString *const XTAsyncSocketSSLProtocolVersionMin;
extern NSString *const XTAsyncSocketSSLProtocolVersionMax;
extern NSString *const XTAsyncSocketSSLSessionOptionFalseStart;
extern NSString *const XTAsyncSocketSSLSessionOptionSendOneByteRecord;
extern NSString *const XTAsyncSocketSSLCipherSuites;
#if !TARGET_OS_IPHONE
extern NSString *const XTAsyncSocketSSLDiffieHellmanParameters;
#endif

#define XTAsyncSocketLoggingContext 65535


typedef NS_ENUM(NSInteger, XTAsyncSocketError) {
	XTAsyncSocketNoError = 0,           // Never used
	XTAsyncSocketBadConfigError,        // Invalid configuration
	XTAsyncSocketBadParamError,         // Invalid parameter was passed
	XTAsyncSocketConnectTimeoutError,   // A connect operation timed out
	XTAsyncSocketReadTimeoutError,      // A read operation timed out
	XTAsyncSocketWriteTimeoutError,     // A write operation timed out
	XTAsyncSocketReadMaxedOutError,     // Reached set maxLength without completing
	XTAsyncSocketClosedError,           // The remote peer closed the connection
	XTAsyncSocketOtherError,            // Description provided in userInfo
};
...
@interface XTAsyncSocket : NSObject

/**
 * XTAsyncSocket uses the standard delegate paradigm,
 ...
 * If you pass NULL, XTAsyncSocket will automatically create it's own socket queue.
...
- (instancetype)initWithDelegate:(nullable id<XTAsyncSocketDelegate>)aDelegate delegateQueue:(nullable dispatch_queue_t)dq;
- (instancetype)initWithDelegate:(nullable id<XTAsyncSocketDelegate>)aDelegate delegateQueue:(nullable dispatch_queue_t)dq socketQueue:(nullable dispatch_queue_t)sq;

/**
 * Create XTAsyncSocket from already connect BSD socket file descriptor
**/
...
@property (atomic, weak, readwrite, nullable) id<XTAsyncSocketDelegate> delegate;
...
- (void)getDelegate:(id<XTAsyncSocketDelegate> __nullable * __nullable)delegatePtr delegateQueue:(dispatch_queue_t __nullable * __nullable)delegateQueuePtr;
- (void)setDelegate:(nullable id<XTAsyncSocketDelegate>)delegate delegateQueue:(nullable dispatch_queue_t)delegateQueue;
...
**/
- (void)synchronouslySetDelegate:(nullable id<XTAsyncSocketDelegate>)delegate;
- (void)synchronouslySetDelegateQueue:(nullable dispatch_queue_t)delegateQueue;
- (void)synchronouslySetDelegate:(nullable id<XTAsyncSocketDelegate>)delegate delegateQueue:(nullable dispatch_queue_t)delegateQueue;

/**
 * By default, both IPv4 and IPv6 are enabled.
 * 
 * For accepting incoming connections, this means XTAsyncSocket automatically supports both protocols,
 * and can simulataneously accept incoming connections on either protocol.
 * 
 * For outgoing connections, this means XTAsyncSocket can connect to remote hosts running either protocol.
 * If a DNS lookup returns only IPv4 results, XTAsyncSocket will automatically use IPv4.
 * If a DNS lookup returns only IPv6 results, XTAsyncSocket will automatically use IPv6.
 ...
 * When a connection is accepted, a new instance of XTAsyncSocket will be spawned to handle it,
 ...
 * 
 * Please note the recommended way of releasing a XTAsyncSocket instance (e.g. in a dealloc method)
 ...
 * it is treated similarly to a timeout - the socket is closed with a XTAsyncSocketReadMaxedOutError.
 ...
 * This is due to the fact that XTAsyncSocket does NOT copy the data. It simply retains it.
 ...
 * 
 * - XTAsyncSocketManuallyEvaluateTrust
 ...
 *     So XTAsyncSocket will invoke the delegate method socket:shouldTrustPeer: passing the SecTrustRef.
 *
 ...
 *
 * - XTAsyncSocketUseCFStreamForTLS (iOS only)
 *     The value must be of type NSNumber, encapsulating a BOOL value.
 *     By default XTAsyncSocket will use the SecureTransport layer to perform encryption.
 ...
 *     
 *     However, if you absolutely must, you can instruct XTAsyncSocket to use the old-fashioned encryption
 *     technique by going through the CFStream instead. So instead of using SecureTransport, XTAsyncSocket
 ...
 *     
 *     Thus all the other keys in the given dictionary will be ignored by XTAsyncSocket,
 ...
 *
 * - XTAsyncSocketSSLProtocolVersionMin
 * - XTAsyncSocketSSLProtocolVersionMax
 ...
 *
 * - XTAsyncSocketSSLDiffieHellmanParameters (Mac OS X only)
 *     The value must be of type NSData.
 ...
 *     You MUST use manual trust evaluation instead (see XTAsyncSocketManuallyEvaluateTrust).
 ...
 *     You MUST use XTAsyncSocketSSLProtocolVersionMin & XTAsyncSocketSSLProtocolVersionMin instead.
...
/**
 * XTAsyncSocket maintains thread safety by using an internal serial dispatch_queue.
 ...
 * This is because the XT API offers no mechanism to discover a queue's targetQueue.
 ...
 * - (dispatch_queue_t)newSocketQueueForConnectionFromAddress:(NSData *)address onSocket:(XTAsyncSocket *)sock
 * {
 ...
 *     return socketQueue;
 * }
 * - (void)socket:(XTAsyncSocket *)sock didAcceptNewSocket:(XTAsyncSocket *)newSocket
 * {
 ...
 *   The faster and more powerful XT API isn't notified properly in this case.
 * 
...
@protocol XTAsyncSocketDelegate <NSObject>
@optional
...
**/
- (nullable dispatch_queue_t)newSocketQueueForConnectionFromAddress:(NSData *)address onSocket:(XTAsyncSocket *)sock;
 ...
 * The host parameter will be an IP address, not a DNS name.
 **/
- (void)socket:(XTAsyncSocket *)sock didConnectToUrl:(NSURL *)url;
...
**/
- (void)socket:(XTAsyncSocket *)sock didReadPartialDataOfLength:(NSUInteger)partialLength tag:(long)tag;
 ...
 * It may be used to for things such as updating progress bars.
**/
- (void)socket:(XTAsyncSocket *)sock didWritePartialDataOfLength:(NSUInteger)partialLength tag:(long)tag;
...
**/
- (NSTimeInterval)socket:(XTAsyncSocket *)sock shouldTimeoutReadWithTag:(long)tag
...
**/
- (NSTimeInterval)socket:(XTAsyncSocket *)sock shouldTimeoutWriteWithTag:(long)tag
                                                                  elapsed:(NSTimeInterval)elapsed
                                                                bytesDone:(NSUInteger)length;
...
**/
- (void)socketDidCloseReadStream:(XTAsyncSocket *)sock;
 ...
 * 
 * Note: If the XTAsyncSocket instance is deallocated while it is still connected,
...
**/
- (void)socket:(XTAsyncSocket *)sock didReceiveTrust:(SecTrustRef)trust
