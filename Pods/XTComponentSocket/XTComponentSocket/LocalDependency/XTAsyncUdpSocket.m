...
@class XTAsyncUdpSendPacket;

NSString *const XTAsyncUdpSocketException = @"XTAsyncUdpSocketException";
NSString *const XTAsyncUdpSocketErrorDomain = @"XTAsyncUdpSocketErrorDomain";

NSString *const XTAsyncUdpSocketQueueName = @"XTAsyncUdpSocket";
NSString *const XTAsyncUdpSocketThreadName = @"XTAsyncUdpSocket-CFStream";

enum XTAsyncUdpSocketFlags
{
...
@interface XTAsyncUdpSocket ()
{
	...
	XTAsyncUdpSocketReceiveFilterBlock receiveFilterBlock;
	dispatch_queue_t receiveFilterQueue;
	BOOL receiveFilterAsync;
	
	XTAsyncUdpSocketSendFilterBlock sendFilterBlock;
	...
	XTAsyncUdpSendPacket *currentSend;
...
/**
 * The XTAsyncUdpSendPacket encompasses the instructions for a single send/write.
**/
@interface XTAsyncUdpSendPacket : NSObject {
@public
	NSData *buffer;
...
@implementation XTAsyncUdpSendPacket
...
@interface XTAsyncUdpSpecialPacket : NSObject {
@public
//	uint8_t type;
...
@implementation XTAsyncUdpSpecialPacket
...
@implementation XTAsyncUdpSocket
...
- (id)initWithDelegate:(id <XTAsyncUdpSocketDelegate>)aDelegate delegateQueue:(dispatch_queue_t)dq
{
	LogTrace();
...
- (id)initWithDelegate:(id <XTAsyncUdpSocketDelegate>)aDelegate delegateQueue:(dispatch_queue_t)dq socketQueue:(dispatch_queue_t)sq
{
	LogTrace();
	...
	if ((self = [super init]))
	{
		...
		{
			socketQueue = dispatch_queue_create([XTAsyncUdpSocketQueueName UTF8String], NULL);
		}
...
- (void)setDelegate:(id <XTAsyncUdpSocketDelegate>)newDelegate synchronously:(BOOL)synchronously
{
...
- (void)setDelegate:(id <XTAsyncUdpSocketDelegate>)newDelegate
{
	[self setDelegate:newDelegate synchronously:NO];
}

- (void)synchronouslySetDelegate:(id <XTAsyncUdpSocketDelegate>)newDelegate
{
...
- (void)getDelegate:(id <XTAsyncUdpSocketDelegate> *)delegatePtr delegateQueue:(dispatch_queue_t *)delegateQueuePtr
{
	if (dispatch_get_specific(IsOnSocketQueueOrTargetQueueKey))
	{
...
- (void)setDelegate:(id <XTAsyncUdpSocketDelegate>)newDelegate delegateQueue:(dispatch_queue_t)newDelegateQueue synchronously:(BOOL)synchronously
{
	dispatch_block_t block = ^{
...
- (void)setDelegate:(id <XTAsyncUdpSocketDelegate>)newDelegate delegateQueue:(dispatch_queue_t)newDelegateQueue
{
	[self setDelegate:newDelegate delegateQueue:newDelegateQueue synchronously:NO];
}

- (void)synchronouslySetDelegate:(id <XTAsyncUdpSocketDelegate>)newDelegate delegateQueue:(dispatch_queue_t)newDelegateQueue
{
...
- (NSError *)badConfigError:(NSString *)errMsg
{
	...
	return [NSError errorWithDomain:XTAsyncUdpSocketErrorDomain
	                           code:XTAsyncUdpSocketBadConfigError
	                       userInfo:userInfo];
}
...
- (NSError *)badParamError:(NSString *)errMsg
{
	...
	return [NSError errorWithDomain:XTAsyncUdpSocketErrorDomain
	                           code:XTAsyncUdpSocketBadParamError
	                       userInfo:userInfo];
}
...
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"XTAsyncUdpSocketSendTimeoutError",
	                                                     @"XTAsyncUdpSocket", [NSBundle mainBundle],
	                                                     @"Send operation timed out", nil);
	
	NSDictionary *userInfo = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];
	
	return [NSError errorWithDomain:XTAsyncUdpSocketErrorDomain
	                           code:XTAsyncUdpSocketSendTimeoutError
	                       userInfo:userInfo];
}

- (NSError *)socketClosedError
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"XTAsyncUdpSocketClosedError",
	                                                     @"XTAsyncUdpSocket", [NSBundle mainBundle],
	                                                     @"Socket closed", nil);
	
	NSDictionary *userInfo = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];
	
	return [NSError errorWithDomain:XTAsyncUdpSocketErrorDomain code:XTAsyncUdpSocketClosedError userInfo:userInfo];
}
...
- (NSError *)otherError:(NSString *)errMsg
{
	...
	return [NSError errorWithDomain:XTAsyncUdpSocketErrorDomain
	                           code:XTAsyncUdpSocketOtherError
	                       userInfo:userInfo];
}
...
- (BOOL)createSocket4:(BOOL)useIPv4 socket6:(BOOL)useIPv6 error:(NSError **)errPtr
{
	...
	int(^createSocket)(int) = ^int (int domain) {
         ...
         *
         * This is the reason of #222(XT does not necessarily return the size of an entire UDP packet) and
         *  #535(XTAsyncUDPSocket can not send data when data is greater than 9K)
         *
         *
...
- (BOOL)connectToHost:(NSString *)host onPort:(uint16_t)port error:(NSError **)errPtr
{
	...
	dispatch_block_t block = ^{ @autoreleasepool {
		...
		XTAsyncUdpSpecialPacket *packet = [[XTAsyncUdpSpecialPacket alloc] init];
...
- (void)maybeConnect
{
	...
	BOOL sendQueueReady = [currentSend isKindOfClass:[XTAsyncUdpSpecialPacket class]];
	
	if (sendQueueReady)
	{
		XTAsyncUdpSpecialPacket *connectPacket = (XTAsyncUdpSpecialPacket *)currentSend;
...
- (void)sendData:(NSData *)data withTimeout:(NSTimeInterval)timeout tag:(long)tag
{
	...
	XTAsyncUdpSendPacket *packet = [[XTAsyncUdpSendPacket alloc] initWithData:data timeout:timeout tag:tag];
...
- (void)sendData:(NSData *)data toAddress:(NSData *)remoteAddr withTimeout:(NSTimeInterval)timeout tag:(long)tag
{
	...
	XTAsyncUdpSendPacket *packet = [[XTAsyncUdpSendPacket alloc] initWithData:data timeout:timeout tag:tag];
	packet->addressFamily = [XTAsyncUdpSocket familyFromAddress:remoteAddr];
...
- (void)setSendFilter:(XTAsyncUdpSocketSendFilterBlock)filterBlock withQueue:(dispatch_queue_t)filterQueue
{
	[self setSendFilter:filterBlock withQueue:filterQueue isAsynchronous:YES];
}

- (void)setSendFilter:(XTAsyncUdpSocketSendFilterBlock)filterBlock
            withQueue:(dispatch_queue_t)filterQueue
       isAsynchronous:(BOOL)isAsynchronous
{
	XTAsyncUdpSocketSendFilterBlock newFilterBlock = NULL;
...
- (void)maybeDequeueSend
{
	...
	if (currentSend == nil)
	{
		...
		while ([sendQueue count] > 0)
		{
			...
			if ([currentSend isKindOfClass:[XTAsyncUdpSpecialPacket class]])
			{
				[self maybeConnect];
...
- (void)doPreSend
{
	...
	if (sendFilterBlock && sendFilterQueue)
	{
		...
		if (sendFilterAsync)
		{
			...
			XTAsyncUdpSendPacket *sendPacket = currentSend;
...
- (void)setReceiveFilter:(XTAsyncUdpSocketReceiveFilterBlock)filterBlock withQueue:(dispatch_queue_t)filterQueue
{
	[self setReceiveFilter:filterBlock withQueue:filterQueue isAsynchronous:YES];
}

- (void)setReceiveFilter:(XTAsyncUdpSocketReceiveFilterBlock)filterBlock
               withQueue:(dispatch_queue_t)filterQueue
          isAsynchronous:(BOOL)isAsynchronous
{
	XTAsyncUdpSocketReceiveFilterBlock newFilterBlock = NULL;
	dispatch_queue_t newFilterQueue = NULL;
...
- (void)doReceive
{
	...
	if (doReceive4)
	{
		...
		// #222: XT does not necessarily return the size of an entire UDP packet 
...
+ (void)listenerThread
{
	@autoreleasepool {
		...
		[[NSThread currentThread] setName:XTAsyncUdpSocketThreadName];
...
+ (void)addStreamListener:(XTAsyncUdpSocket *)asyncUdpSocket
{
	LogTrace();
...
+ (void)removeStreamListener:(XTAsyncUdpSocket *)asyncUdpSocket
{
	LogTrace();
	NSAssert([NSThread currentThread] == listenerThread, @"Invoked on wrong thread");
...
{
	@autoreleasepool {
		XTAsyncUdpSocket *asyncUdpSocket = (__bridge XTAsyncUdpSocket *)pInfo;
