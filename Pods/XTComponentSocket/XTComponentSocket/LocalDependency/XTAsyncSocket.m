...
#ifndef XTAsyncSocketLoggingEnabled
#define XTAsyncSocketLoggingEnabled 0
#endif

#if XTAsyncSocketLoggingEnabled

// Logging Enabled - See log level below

// Logging uses the CocoaLumberjack framework (which is also XT based).
...
#define LogAsync   YES
#define LogContext XTAsyncSocketLoggingContext
...
#ifndef XTAsyncSocketLogLevel
#define XTAsyncSocketLogLevel LOG_LEVEL_VERBOSE
#endif

// Log levels : off, error, warn, info, verbose
static const int logLevel = XTAsyncSocketLogLevel;
...
NSString *const XTAsyncSocketException = @"XTAsyncSocketException";
NSString *const XTAsyncSocketErrorDomain = @"XTAsyncSocketErrorDomain";

NSString *const XTAsyncSocketQueueName = @"XTAsyncSocket";
NSString *const XTAsyncSocketThreadName = @"XTAsyncSocket-CFStream";

NSString *const XTAsyncSocketManuallyEvaluateTrust = @"XTAsyncSocketManuallyEvaluateTrust";
#if TARGET_OS_IPHONE
NSString *const XTAsyncSocketUseCFStreamForTLS = @"XTAsyncSocketUseCFStreamForTLS";
#endif
NSString *const XTAsyncSocketSSLPeerID = @"XTAsyncSocketSSLPeerID";
NSString *const XTAsyncSocketSSLProtocolVersionMin = @"XTAsyncSocketSSLProtocolVersionMin";
NSString *const XTAsyncSocketSSLProtocolVersionMax = @"XTAsyncSocketSSLProtocolVersionMax";
NSString *const XTAsyncSocketSSLSessionOptionFalseStart = @"XTAsyncSocketSSLSessionOptionFalseStart";
NSString *const XTAsyncSocketSSLSessionOptionSendOneByteRecord = @"XTAsyncSocketSSLSessionOptionSendOneByteRecord";
NSString *const XTAsyncSocketSSLCipherSuites = @"XTAsyncSocketSSLCipherSuites";
#if !TARGET_OS_IPHONE
NSString *const XTAsyncSocketSSLDiffieHellmanParameters = @"XTAsyncSocketSSLDiffieHellmanParameters";
#endif

enum XTAsyncSocketFlags
{
...
@interface XTAsyncSocketPreBuffer : NSObject
{
	uint8_t *preBuffer;
...
@implementation XTAsyncSocketPreBuffer
...
/**
 * The XTAsyncReadPacket encompasses the instructions for any given read.
 * The content of a read packet allows the code to determine if we're:
 *  - reading to a certain length
 *  - reading to a certain separator
 *  - or simply reading the first chunk of available data
**/
@interface XTAsyncReadPacket : NSObject
{
  @public
...
- (NSUInteger)readLengthForTermWithPreBuffer:(XTAsyncSocketPreBuffer *)preBuffer found:(BOOL *)foundPtr;

- (NSInteger)searchForTermAfterPreBuffering:(ssize_t)numBytes;

@end

@implementation XTAsyncReadPacket
...
/**
 * The XTAsyncWritePacket encompasses the instructions for any given write.
**/
@interface XTAsyncWritePacket : NSObject
{
  @public
	NSData *buffer;
	NSUInteger bytesDone;
...
@implementation XTAsyncWritePacket
...
/**
 * The XTAsyncSpecialPacket encompasses special instructions for interruptions in the read/write queues.
 * This class my be altered to support more than just TLS in the future.
**/
@interface XTAsyncSpecialPacket : NSObject
{
  @public
...
@implementation XTAsyncSpecialPacket
...
@implementation XTAsyncSocket
{
	uint32_t flags;
	uint16_t config;
	
	__weak id<XTAsyncSocketDelegate> delegate;
	...
	XTAsyncReadPacket *currentRead;
	XTAsyncWritePacket *currentWrite;
	
	unsigned long socketFDBytesAvailable;
	
	XTAsyncSocketPreBuffer *preBuffer;
	...
	XTAsyncSocketPreBuffer *sslPreBuffer;
...
- (id)initWithDelegate:(id<XTAsyncSocketDelegate>)aDelegate delegateQueue:(dispatch_queue_t)dq socketQueue:(dispatch_queue_t)sq
{
	if((self = [super init]))
	{
		delegate = aDelegate;
		...
		{
			socketQueue = dispatch_queue_create([XTAsyncSocketQueueName UTF8String], NULL);
		}
		...
		preBuffer = [[XTAsyncSocketPreBuffer alloc] initWithCapacity:(1024 * 4)];
...
+ (nullable instancetype)socketFromConnectedSocketFD:(int)socketFD delegate:(nullable id<XTAsyncSocketDelegate>)aDelegate delegateQueue:(nullable dispatch_queue_t)dq error:(NSError**)error {
  return [self socketFromConnectedSocketFD:socketFD delegate:aDelegate delegateQueue:dq socketQueue:NULL error:error];
}

+ (nullable instancetype)socketFromConnectedSocketFD:(int)socketFD delegate:(nullable id<XTAsyncSocketDelegate>)aDelegate delegateQueue:(nullable dispatch_queue_t)dq socketQueue:(nullable dispatch_queue_t)sq error:(NSError**)error
{
  __block BOOL errorOccured = NO;
  
  XTAsyncSocket *socket = [[[self class] alloc] initWithDelegate:aDelegate delegateQueue:dq socketQueue:sq];
  ...
  dispatch_sync(socket->socketQueue, ^{ @autoreleasepool {
    ...
    if (retVal)
    {
      NSString *errMsg = NSLocalizedStringWithDefaultValue(@"XTAsyncSocketOtherError",
                                                           @"XTAsyncSocket", [NSBundle mainBundle],
      ...
      errorOccured = YES;
      if (error)
        *error = [NSError errorWithDomain:XTAsyncSocketErrorDomain code:XTAsyncSocketOtherError userInfo:userInfo];
      return;
    }
...
- (void)getDelegate:(id<XTAsyncSocketDelegate> *)delegatePtr delegateQueue:(dispatch_queue_t *)delegateQueuePtr
{
	if (dispatch_get_specific(IsOnSocketQueueOrTargetQueueKey))
	{
...
- (BOOL)acceptOnInterface:(NSString *)inInterface port:(uint16_t)port error:(NSError **)errPtr
{
	...
	dispatch_block_t block = ^{ @autoreleasepool {
		...
		if (enableIPv4)
		{
			...
			__weak XTAsyncSocket *weakSelf = self;
			
			dispatch_source_set_event_handler(accept4Source, ^{ @autoreleasepool {
			#pragma clang diagnostic push
			#pragma clang diagnostic warning "-Wimplicit-retain-self"
				
				__strong XTAsyncSocket *strongSelf = weakSelf;
...
- (BOOL)doAccept:(int)parentSocketFD
{
	...
	if (delegateQueue)
	{
		...
		dispatch_async(delegateQueue, ^{ @autoreleasepool {
			...
			// Create XTAsyncSocket instance for accepted socket
			
			XTAsyncSocket *acceptedSocket = [[[self class] alloc] initWithDelegate:theDelegate
...
- (NSError *)badConfigError:(NSString *)errMsg
{
	...
	return [NSError errorWithDomain:XTAsyncSocketErrorDomain code:XTAsyncSocketBadConfigError userInfo:userInfo];
}
...
- (NSError *)badParamError:(NSString *)errMsg
{
	...
	return [NSError errorWithDomain:XTAsyncSocketErrorDomain code:XTAsyncSocketBadParamError userInfo:userInfo];
}
...
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"XTAsyncSocketConnectTimeoutError",
	                                                     @"XTAsyncSocket", [NSBundle mainBundle],
	                                                     @"Attempt to connect to host timed out", nil);
	
	NSDictionary *userInfo = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];
	
	return [NSError errorWithDomain:XTAsyncSocketErrorDomain code:XTAsyncSocketConnectTimeoutError userInfo:userInfo];
}
...
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"XTAsyncSocketReadMaxedOutError",
														 @"XTAsyncSocket", [NSBundle mainBundle],
														 @"Read operation reached set maximum length", nil);
	
	NSDictionary *info = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];
	
	return [NSError errorWithDomain:XTAsyncSocketErrorDomain code:XTAsyncSocketReadMaxedOutError userInfo:info];
}
...
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"XTAsyncSocketReadTimeoutError",
	                                                     @"XTAsyncSocket", [NSBundle mainBundle],
	                                                     @"Read operation timed out", nil);
	
	NSDictionary *userInfo = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];
	
	return [NSError errorWithDomain:XTAsyncSocketErrorDomain code:XTAsyncSocketReadTimeoutError userInfo:userInfo];
}
...
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"XTAsyncSocketWriteTimeoutError",
	                                                     @"XTAsyncSocket", [NSBundle mainBundle],
	                                                     @"Write operation timed out", nil);
	
	NSDictionary *userInfo = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];
	
	return [NSError errorWithDomain:XTAsyncSocketErrorDomain code:XTAsyncSocketWriteTimeoutError userInfo:userInfo];
}

- (NSError *)connectionClosedError
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"XTAsyncSocketClosedError",
	                                                     @"XTAsyncSocket", [NSBundle mainBundle],
	                                                     @"Socket closed by remote peer", nil);
	
	NSDictionary *userInfo = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];
	
	return [NSError errorWithDomain:XTAsyncSocketErrorDomain code:XTAsyncSocketClosedError userInfo:userInfo];
}
...
- (NSError *)otherError:(NSString *)errMsg
{
	...
	return [NSError errorWithDomain:XTAsyncSocketErrorDomain code:XTAsyncSocketOtherError userInfo:userInfo];
}
...
- (BOOL)usingCFStreamForTLS
{
	...
	{
		// The startTLS method was given the XTAsyncSocketUseCFStreamForTLS flag.
                        ...
                        tag:(long)tag
{
	...
	XTAsyncReadPacket *packet = [[XTAsyncReadPacket alloc] initWithData:buffer
	                                                          startOffset:offset
	                                                            maxLength:length
	                                                              timeout:timeout
	                                                           readLength:0
...
- (float)progressOfReadReturningTag:(long *)tagPtr bytesDone:(NSUInteger *)donePtr total:(NSUInteger *)totalPtr
{
	...
	dispatch_block_t block = ^{
		...
		if (!currentRead || ![currentRead isKindOfClass:[XTAsyncReadPacket class]])
		{
...
- (void)maybeDequeueRead
{
	...
	if ((currentRead == nil) && (flags & kConnected))
	{
		if ([readQueue count] > 0)
		{
			...
			if ([currentRead isKindOfClass:[XTAsyncSpecialPacket class]])
			{
				LogVerbose(@"Dequeued XTAsyncSpecialPacket");
			...
			{
				LogVerbose(@"Dequeued XTAsyncReadPacket");
...
- (void)doReadData
{
	...
	if ([self usingCFStreamForTLS])
	{
		...
		// Requested CFStream, rather than SecureTransport, for TLS (via XTAsyncSocketUseCFStreamForTLS)
...
- (void)completeCurrentRead
{
	...
	{
		XTAsyncReadPacket *theRead = currentRead; // Ensure currentRead retained since result may not own buffer
...
- (void)doReadTimeout
{
	...
	{
		XTAsyncReadPacket *theRead = currentRead;
...
- (void)writeData:(NSData *)data withTimeout:(NSTimeInterval)timeout tag:(long)tag
{
	...
	XTAsyncWritePacket *packet = [[XTAsyncWritePacket alloc] initWithData:data timeout:timeout tag:tag];
...
- (float)progressOfWriteReturningTag:(long *)tagPtr bytesDone:(NSUInteger *)donePtr total:(NSUInteger *)totalPtr
{
	...
	dispatch_block_t block = ^{
		...
		if (!currentWrite || ![currentWrite isKindOfClass:[XTAsyncWritePacket class]])
		{
...
- (void)maybeDequeueWrite
{
	...
	if ((currentWrite == nil) && (flags & kConnected))
	{
		if ([writeQueue count] > 0)
		{
			...
			if ([currentWrite isKindOfClass:[XTAsyncSpecialPacket class]])
			{
				LogVerbose(@"Dequeued XTAsyncSpecialPacket");
			...
			{
				LogVerbose(@"Dequeued XTAsyncWritePacket");
...
- (void)doWriteData
{
	...
	// Note: This method is not called if currentWrite is a XTAsyncSpecialPacket (startTLS packet)
...
- (void)doWriteTimeout
{
	...
	{
		XTAsyncWritePacket *theWrite = currentWrite;
...
- (void)startTLS:(NSDictionary *)tlsSettings
{
	...
	XTAsyncSpecialPacket *packet = [[XTAsyncSpecialPacket alloc] initWithTLSSettings:tlsSettings];
...
- (void)maybeStartTLS
{
	...
	if ((flags & kStartingReadTLS) && (flags & kStartingWriteTLS))
	{
		...
		#if TARGET_OS_IPHONE
		{
			XTAsyncSpecialPacket *tlsPacket = (XTAsyncSpecialPacket *)currentRead;
            NSDictionary *tlsSettings = @{};
            if (tlsPacket) {
                tlsSettings = tlsPacket->tlsSettings;
            }
			NSNumber *value = [tlsSettings objectForKey:XTAsyncSocketUseCFStreamForTLS];
...
{
	XTAsyncSocket *asyncSocket = (__bridge XTAsyncSocket *)connection;
...
- (void)ssl_startTLS
{
	...
	BOOL shouldManuallyEvaluateTrust = [[tlsSettings objectForKey:XTAsyncSocketManuallyEvaluateTrust] boolValue];
	...
	//  3. XTAsyncSocketSSLPeerID
	//  4. XTAsyncSocketSSLProtocolVersionMin
	//  5. XTAsyncSocketSSLProtocolVersionMax
	//  6. XTAsyncSocketSSLSessionOptionFalseStart
	//  7. XTAsyncSocketSSLSessionOptionSendOneByteRecord
	//  8. XTAsyncSocketSSLCipherSuites
	//  9. XTAsyncSocketSSLDiffieHellmanParameters (Mac)
	//
	...
	value = [tlsSettings objectForKey:XTAsyncSocketSSLPeerID];
	...
	{
		NSAssert(NO, @"Invalid value for XTAsyncSocketSSLPeerID. Value must be of type NSData."
		             @" (You can convert strings to data using a method like"
		             @" [string dataUsingEncoding:NSUTF8StringEncoding])");
		
		[self closeWithError:[self otherError:@"Invalid value for XTAsyncSocketSSLPeerID."]];
		return;
	}
	
	// 4. XTAsyncSocketSSLProtocolVersionMin
	
	value = [tlsSettings objectForKey:XTAsyncSocketSSLProtocolVersionMin];
	...
	{
		NSAssert(NO, @"Invalid value for XTAsyncSocketSSLProtocolVersionMin. Value must be of type NSNumber.");
		
		[self closeWithError:[self otherError:@"Invalid value for XTAsyncSocketSSLProtocolVersionMin."]];
		return;
	}
	
	// 5. XTAsyncSocketSSLProtocolVersionMax
	
	value = [tlsSettings objectForKey:XTAsyncSocketSSLProtocolVersionMax];
	if ([value isKindOfClass:[NSNumber class]])
	{
	...
	{
		NSAssert(NO, @"Invalid value for XTAsyncSocketSSLProtocolVersionMax. Value must be of type NSNumber.");
		
		[self closeWithError:[self otherError:@"Invalid value for XTAsyncSocketSSLProtocolVersionMax."]];
		return;
	}
	
	// 6. XTAsyncSocketSSLSessionOptionFalseStart
	
	value = [tlsSettings objectForKey:XTAsyncSocketSSLSessionOptionFalseStart];
	if ([value isKindOfClass:[NSNumber class]])
	{
	...
	{
		NSAssert(NO, @"Invalid value for XTAsyncSocketSSLSessionOptionFalseStart. Value must be of type NSNumber.");
		
		[self closeWithError:[self otherError:@"Invalid value for XTAsyncSocketSSLSessionOptionFalseStart."]];
		return;
	}
	
	// 7. XTAsyncSocketSSLSessionOptionSendOneByteRecord
	
	value = [tlsSettings objectForKey:XTAsyncSocketSSLSessionOptionSendOneByteRecord];
	if ([value isKindOfClass:[NSNumber class]])
	{
	...
	{
		NSAssert(NO, @"Invalid value for XTAsyncSocketSSLSessionOptionSendOneByteRecord."
		             @" Value must be of type NSNumber.");
		
		[self closeWithError:[self otherError:@"Invalid value for XTAsyncSocketSSLSessionOptionSendOneByteRecord."]];
		return;
	}
	
	// 8. XTAsyncSocketSSLCipherSuites
	
	value = [tlsSettings objectForKey:XTAsyncSocketSSLCipherSuites];
	if ([value isKindOfClass:[NSArray class]])
	{
	...
	{
		NSAssert(NO, @"Invalid value for XTAsyncSocketSSLCipherSuites. Value must be of type NSArray.");
		
		[self closeWithError:[self otherError:@"Invalid value for XTAsyncSocketSSLCipherSuites."]];
		return;
	}
	
	// 9. XTAsyncSocketSSLDiffieHellmanParameters
	
	#if !TARGET_OS_IPHONE
	value = [tlsSettings objectForKey:XTAsyncSocketSSLDiffieHellmanParameters];
	if ([value isKindOfClass:[NSData class]])
	{
	...
	{
		NSAssert(NO, @"Invalid value for XTAsyncSocketSSLDiffieHellmanParameters. Value must be of type NSData.");
		
		[self closeWithError:[self otherError:@"Invalid value for XTAsyncSocketSSLDiffieHellmanParameters."]];
		return;
	}
	#endif
	...
	if (value)
	{
		NSAssert(NO, @"Security option unavailable - kCFStreamSSLLevel"
		             @" - You must use XTAsyncSocketSSLProtocolVersionMin & XTAsyncSocketSSLProtocolVersionMax");
	...
	sslPreBuffer = [[XTAsyncSocketPreBuffer alloc] initWithCapacity:(1024 * 4)];
...
- (void)ssl_continueSSLHandshake
{
	...
	else if (status == errSSLPeerAuthCompleted)
	{
		...
		else
		{
			...
			NSString *msg = @"XTAsyncSocketManuallyEvaluateTrust specified in tlsSettings,"
...
- (void)cf_startTLS
{
	...
	NSAssert([currentRead isKindOfClass:[XTAsyncSpecialPacket class]], @"Invalid read packet for startTLS");
	NSAssert([currentWrite isKindOfClass:[XTAsyncSpecialPacket class]], @"Invalid write packet for startTLS");
...
+ (void)startCFStreamThreadIfNeeded
{
	...
	dispatch_once(&predicate, ^{
		...
		cfstreamThreadSetupQueue = dispatch_queue_create("XTAsyncSocket-CFStreamThreadSetup", DISPATCH_QUEUE_SERIAL);
	});
...
{
	[[NSThread currentThread] setName:XTAsyncSocketThreadName];
...
+ (void)scheduleCFStreams:(XTAsyncSocket *)asyncSocket
{
	LogTrace();
...
+ (void)unscheduleCFStreams:(XTAsyncSocket *)asyncSocket
{
	LogTrace();
	NSAssert([NSThread currentThread] == cfstreamThread, @"Invoked on wrong thread");
...
{
	XTAsyncSocket *asyncSocket = (__bridge XTAsyncSocket *)pInfo;
